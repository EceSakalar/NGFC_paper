...
bs_name='B'; bs_num='200'; bs_exp='b'; bs_typ='sp '; %basic data entry
ch_dwnsmpl=25;                                       %give the downsampling rate
ch_rate=10000; ch_rate=ch_rate/ch_dwnsmpl;           %give the silicon probe sampling rate (in Hz)

un_chans=[130:164];                                                           %channels containing the spike times for the units
un_nums=[8,18,19,23,27,30,205,47,53,68,70,71,78,85,207,238,95,100,138,139,196,199,211,213,232,142,145,157,158,164,165,179,181,216,218];          %unit numbers (in kwik file)
%shanks from which unit was isolated; shank 0 is glass electrode and allows shortening of the recording time
un_shnk =[1,2,1,2,2,2,1,3,4,4,3,3,4,4,4,4,5,6,6,6,5,6,6,5,5,7,7,7,7,8,8,8,8,7,7];                              

sh_pyr=5;
ch_pyr=13;                                      %channel number (ventralmost LFP contact is 1) for the pyramidal layer
...
for fl_indx=1:length(fl_ord); %FILE STEPPER
    ...
    lfp_name=strcat(bs_name, bs_num, bs_exp, num2str(fl_ord(fl_indx)),'sh',num2str(sh_pyr),'LFP.mat');
    load(lfp_name);
