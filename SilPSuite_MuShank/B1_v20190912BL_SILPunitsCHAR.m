%This program reads original .dat as created from INTAN files (assumes .dat files encode directly µV
...
dt_name='B196aAllKilo.dat';

bs_name='B'; bs_num='196'; bs_exp='a'; bs_typ='sp'; bs_dayname='d1';

ch_n = 64;                                      %number of channels in the dat files
ch_rate = 20000;                                        %give the silicon probe sampling rate (in Hz)
%channels in the dat file with the maximum amplitude
ch_max = [4,5,5,5,7,8,9,10,5,8,28,29,29,29,30,30,30,32,43,44,44,44,44,46,47,44,40,39,49,50,50,51,51,52,52,53,53,54,57,60,53,62];
%channels in the spike2 files and originating tetrodes
ch_uns = [65:106];
%unit numbers (in kwik file)
un_nums=[0,1,3,4,13,16,19,21,129,142,45,47,48,49,51,52,53,144,74,76,77,81,87,90,92,133,141,143,94,95,97,102,104,105,106,107,110,113,116,118,138,146];                 
%shank identifier (should match the dat file order)
un_shnk =[repmat([1],1,10) repmat([2],1,8) repmat([3],1,10)  repmat([4],1,14) repmat([5],1,0) repmat([6],1,0) repmat([7],1,0)  repmat([8],1,0)];                       %shanks from which unit was isolated
    ...
    for pt_indx = 1:length(ch_uns)
        ...
        if pr_indx~=pt_indx && char_intact{pr_indx,pt_indx,1}==1 && max(((char_xcor{pr_indx,pt_indx}(91:201,1)).*length(spks{pr_indx}))./1000)>=15;
            ...
            if pl_indx<5
                ...
                fg_name=strcat('fig',num2str(bs_num),num2str(bs_exp),'_CHARunit_intP',num2str(fg_indx));
                saveas(gcf,fg_name)
                clear fg_name
...
save(writename, 'datum', 'un_shnk','ch_uns', 'un_nums','char_units','char_intact','char_intactQ','char_xcor','t_rec','spks','-v7.3');
