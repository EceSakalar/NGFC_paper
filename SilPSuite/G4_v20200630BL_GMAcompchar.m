...
%basic data entry
bs.name='B'; bs.num='192'; bs.unit='d'; bs.typ='sx'; 
bs.flord=[1];

ch.pyrch=15;                                        %channel number (ventralmost LFP contact is 1) for the pyramidal layer
...
%GAMMA properties
gm.comp = 'CA1slw';                                                        %name of gamma component
gm.chan = 9;                                 %CSD channel for the actual gamma component for each shank (write NaN for not include)
gm.cfrq = [31.2704, 37.6176];                                              %band pass filter corner frequencies (low and high, included in the boundary)
gm.ncoeff = 1024;
gm.rate = smp.rate;
...
for fl_indx=1:length(bs.flord) %FILE STEPPER
    ...
    csd_name=strcat(bs.name, bs.num, bs.unit, num2str(bs.flord(fl_indx)),'CSD.mat');
    load(csd_name);
    gmCSD_file = CSD(gm.chan,:)*-1; clear CSD csd_name;
