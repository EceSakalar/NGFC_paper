...
gm.chan = 5;                                 %CSD channel for the actual gamma component for each shank (write NaN for not include)
gm.cfrq = [52.9801,83.7696];                                              %band pass filter corner frequencies (low and high, included in the boundary)
gm.ncoeff = 1024;
gm.rate = 2000;
...
csd_name=strcat(bs.name, 'CSD.mat');
load(csd_name);
...
fl_name=strcat(bs.name, 'WAW.mat'); %file name
ch_name=gs.spkst;
...
figname=strcat(bs.name,'_gSyn_gammaMcyc_', bs.cell);
figure('Name', figname, 'Numbertitle', 'off', 'PaperOrientation', 'portrait', 'PaperType', 'A4')
subplot(3,1,1);
title(strcat(bs.name,' gSyn during a gammaMcycle; unit:', bs.cell,'; all cycles with spike'));
...
%saving figure
fg_name=strcat('fig',bs.name,'_',bs.cell,'_gSYNgammaMcyc.fig');
saveas(gcf,fg_name)
clear fg_name figname

%saving the collected data
datum = date;
writename=strcat(bs.name, '_gSYNgammaM_cell_', bs.cell);
