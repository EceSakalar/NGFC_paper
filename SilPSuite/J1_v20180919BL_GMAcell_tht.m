...
bs_name='B'; bs_num='186'; bs_exp='d'; bs_typ='sx';                 %unit basic data entry

ch_n=16;                                                            %number of channels
ch_rate=2000;                                                       %give the silicon probe sampling rate (in Hz)

un_chans=[35:53,55];                                                      %channels containing the spike times for the units
un_nums=[0,5,7,19,23,77,85,104,170,172,202,209,211,50,56,58,73,195,196,1];                                                        %unit numbers (in kwik file)
un_shank =[repmat([1],1,13),repmat([2],1,2),repmat([3],1,4),0];                       %shanks from which unit was isolated
...
for un_indx=1:length(un_chans)
    ...
    %saving figure
    fg_name=strcat('fig',bs_name, bs_num, bs_exp,'GMAcellTHT_shk', num2str(un_shank(un_indx)),'u',num2str(un_nums(un_indx)));
    saveas(gcf,fg_name)
    clear fg_name
end
