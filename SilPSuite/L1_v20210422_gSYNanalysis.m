...
bs.name='B'; bs.num='207'; bs.unit='b'; bs.typ='sp'; bs.cell='s5u50';
bs.shnk=['sh1'];   % put here the shank where the theta is taken from, or leave it empty if shank number is not applicable
bs.flord=[1];

ch.pyrch=16;                                        %channel number (ventralmost LFP contact is 1) for the pyramidal layer
smp.rate=10000;                                      %give the sampling rate of the channels (LFP channel in original LFP.mat file)
...
for fl_indx=1:length(bs.flord) %FILE STEPPER
    ...
    lfp_name=strcat(bs.name, bs.num, bs.unit, num2str(bs.flord(fl_indx)),bs.shnk,'LFP.mat');
    load(lfp_name);
    ...
    %read the gSyn trace
    fl_name=strcat(bs.name, bs.num, bs.unit, num2str(bs.flord(fl_indx)),'Gsyn.mat');
    ...
    ch_name=strcat(bs.name, bs.typ, bs.num, bs.unit,'_Ch', num2str(gs.cycev)); ch_load=load(num2str(fl_name),num2str(ch_name));
    ...
    %accumulating LFP traces, theta segments gSYN traces gSYN events and file segmentation points
    if fl_indx==1
        ...
        thPER = [thPER; thPER_file+((flSEG(fl_indx-1))/smp.rate)];
        gsEVN = [gsEVN; fl_gevn + ((flSEG(fl_indx-1))/smp.rate)];
        gsTRC = [gsTRC fl_gsyn'];
    end; clear  thLFP_file thPER_file fl_gsyn fl_gevn 
...
figname=strcat(bs.name, bs.num, bs.unit,'gSYN_analysis');
...
subplot(3,2,5)
axis off
actext=strcat(bs.name, bs.num, bs.unit,' cell:',bs.cell);
...
%saving figure
fg_name=strcat('fig',bs.name, bs.num, bs.unit,'_cell',bs.cell,'_gSYNanalys');
...
datum = date;
writename=strcat(bs.name, bs.num, bs.unit, '_gSYNfor_cell_', bs.cell);
