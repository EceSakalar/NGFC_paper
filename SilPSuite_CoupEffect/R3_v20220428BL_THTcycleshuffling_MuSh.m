...
%basic data entry
bs.name='B'; bs.num='207'; bs.exp='b'; bs.typ='sp';                 %unit basic data entry
gm.comp = 'CA1mid'; 

%units data entry
un.chan=[146];                                   %channels containing the spike times for the units to shuffle
%unit numbers;
un.nums=[50];                                    %unit numbers (in kwik file)
%shanks from which unit was isolated;
un.shnk=[5];                       %shanks from which unit was isolated
%shank used for reference gamma oscillation for the shuffled cell
un.rfsh=[4];
...
for sp_indx=1:size(spks,1)
    if isnan(spks(sp_indx,3))
        ...
        spks(sp_indx,5) = gm_shank(1,cyc_ind)/smp.rate;
        spks(sp_indx,6) = gm_shank(3,cyc_ind);
        spks(sp_indx,7) = gm_shank(4,cyc_ind);
        spks(sp_indx,8) = gm_shank(7,cyc_ind);
...
for sp_indx=1:size(spks_sh,1)
    if isnan(spks_sh(sp_indx,3))
        ...
        %5-8 closest gamma cycle properties
        [~,cyc_ind]=min(abs((gm_shank(1,:)./smp.rate)-sp_time));
        spks_sh(sp_indx,5) = gm_shank(1,cyc_ind)/smp.rate;
        spks_sh(sp_indx,6) = gm_shank(3,cyc_ind);
        spks_sh(sp_indx,7) = gm_shank(4,cyc_ind);
        spks_sh(sp_indx,8) = gm_shank(7,cyc_ind);
