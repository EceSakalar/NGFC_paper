...
bs.name=['B']; bs.num=['184']; bs.exp=['c'];bs.ngfc=['s2u26'];
an.units=[4,5,6,7,13,18,19,20,22,23,25,26,27,29,30,31,33,34,35,36,37,38,39,40]; %give the PYR units included in the current session 

an.ngfc=[11]; %give the number (sorrend szam!) for the NGFC

fl_name=strcat(bs.name,bs.num,bs.exp,'_gammaunits_CA1slw.mat');
...
fl_name=strcat(bs.name,bs.num,bs.exp,'_gammacomponent_CA1slw.mat');
%load the file for the appropriate gamma analysis (only the spks variable)
load(fl_name,'output');
load(fl_name,'input');
clear fl_name
...
datum = date;
writename=strcat(bs.name, bs.num, bs.exp,bs.ngfc,'_gCA1slw_PYRcoup_NGFCtriggered' );
