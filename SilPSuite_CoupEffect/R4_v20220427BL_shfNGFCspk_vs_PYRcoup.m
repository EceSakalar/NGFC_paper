%This program reads shuffled spike trains for given NGFCs and also pooled pyramidal cells
%same experiment but to be imported from 2 different files!!!) and calculates the PYR cell
%coupling to mid-gamma oscillations before and after NGFC firing
%NGFC spike inclusion criterion: previous spike not closer than 51,5 ms (gSyn declines below 0.35)

clear all

%EXPERIMENT identification data
bs.name=['B']; bs.num=['193']; bs.exp=['b'];bs.ngfc=['s2u33_all'];
an.units=[3,4,5,7,9,10,11,12,13,18,19,21,22,23,24,25,26,27,28,30,31]; %give the PYR units included in the current session 
shf.ngfc=['_un33sh2']; %give the name of the NGFC in the shuffled file (underscore before)
...
%NGFC spikes
fl_name=strcat(bs.name,bs.num,bs.exp,shf.ngfc,'_gammaunits_shuffleCA1mid.mat');
%load the file for the shuffled NGFC spikes (only the spks_sh variable)
load(fl_name,'spks_sh'); clear fl_name
spks_ngf=spks_sh;
clear spks
...
masolashoz=[pyrstat.Rsel'; pyrstat.Ssel';pyrstat.Msel';pyrstat.Nsel'];
%saving the relevant data
datum = date;
writename=strcat(bs.name, bs.num, bs.exp,bs.ngfc,'_gCA1mid_PYRcoup_NGFCshuffled' );
